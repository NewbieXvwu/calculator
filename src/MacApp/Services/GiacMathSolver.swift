...
//
// 诚实性契约（TODO S3，元规则 M4）：任何字段在无法可靠计算时必须进
...
enum GiacMathSolver {
    ...
    static func analyze(_ expr: GraphExpression, params: [String: Double] = [:], trig: GraphTrigMode = .radians) -> GiacFunctionAnalysis {
        ...
        if zerosQ.output == nil || zerosQ.approximate {
            ...
            if items.count > maxSolutionCount {
                tooComplex.insert(.xIntercept) // R4：数值点洪水不冒充完备列表
            } else {
        ...
        if stationaryItems.count > maxSolutionCount {
        ...
        if vaTrusted {
            if let di = domainInfo {
                ...
                // 定义域不可信（截断/失败）→ 奇点集合未知，VA 列表可能不完备。
                vaTrusted = false
            }
        }
        if vaTrusted {
        ...
        func attainedY(at exact: String) -> ValueCandidate? {
        ...
        if structureTrusted {
            for p in numericStationary {
                ...
                guard let y = attainedY(at: p.exact) else {
                    extremaProblem = true
                    continue
                }
            ...
            if let di = domainInfo {
                ...
                for case let (bp, isLower)? in ends {
                    guard let y = attainedY(at: bp.exact) else {
                        extremaProblem = true
                        continue
                    }
                    attainedValues.append(y)
        ...
        if let range = computeRange(
            ...
            attainedValues: attainedValues, infLimitRaw: infLimitRaw) {
            a.range = range
        } else {
        ...
        infLimitRaw: [String: String]) -> String?
    {
        ...
        func attainedCandidate(at exact: String) -> ValueCandidate? {
            guard let y = query("normal(subst(\(f),x=\(exact)))").output,
                  isDisplayableFinite(y), let v = numericValue(y) else { return nil }
            return ValueCandidate(exact: y, value: v, attained: true)
        }
        ...
        for i in 0..<(edges.count - 1) {
            ...
            if let bp = lo.bp {
                guard let c = lo.included ? attainedCandidate(at: bp.exact) : limitCandidate(at: bp.exact, fromRight: true) else { return nil }
                cands.append(c)
            } else {
            ...
            if let bp = hi.bp {
                guard let c = hi.included ? attainedCandidate(at: bp.exact) : limitCandidate(at: bp.exact, fromRight: false) else { return nil }
                cands.append(c)
            } else {
                guard let c = infinityCandidate("inf") else { return nil }
                cands.append(c)
            ...
            for p in numericStationary + nondiff {
                ...
                if let c = attainedCandidate(at: p.exact) {
                    cands.append(c)
                } else {
                    return nil
                }
    ...
    /// "x<>c" 及其 and 连接。解析失败返回 nil（依赖它的字段降级 too complex）。
    private static func parseDomain(_ raw: String) -> DomainInfo? {
        var info = DomainInfo()
        if raw == "x" { return info }
        ...
        while i < chars.count {
            ...
            if c == "(" || c == "[" { depth += 1 }
            if c == ")" || c == "]" { depth -= 1 }
            if depth == 0, c == " ", raw.dropFirst(i).hasPrefix(" and ") {
        ...
        for var term in terms {
            ...
            while term.hasPrefix("("), term.hasSuffix(")"), isBalanced(String(term.dropFirst().dropLast())) {
                term = String(term.dropFirst().dropLast()).trimmingCharacters(in: .whitespaces)
            }
            guard term.hasPrefix("x") else { return nil }
            var rest = String(term.dropFirst())
            let op: String
            ...
            rest = String(rest.dropFirst(op.count)).trimmingCharacters(in: .whitespaces)
            while rest.hasPrefix("("), rest.hasSuffix(")"), isBalanced(String(rest.dropFirst().dropLast())) {
                rest = String(rest.dropFirst().dropLast())
            }
            guard let v = numericValue(rest) else { return nil }
            let bp = Breakpoint(exact: rest, value: v)
            switch op {
            case ">=", ">":
    ...
    static func isDisplayableFinite(_ s: String) -> Bool {
        ...
        while let r = s.range(of: #"[A-Za-z_][A-Za-z_0-9]*"#, options: .regularExpression, range: index..<s.endIndex) {
            if !allowed.contains(String(s[r])) { return false }
            index = r.upperBound
        }
        return true
    }
